...
@implementation WAFnetwokingTools
+ (void)post:(NSString *)url params:(id)params progress:(void (^)(NSProgress *progress))progress success:(void (^)(id responseObject))success failure:(void (^)(NSError *error))failure
{
    ...
    NSString *national = [NSString string];
    NSArray *appLanguages = [[NSUserDefaults standardUserDefaults] objectForKey:@"AppleLanguages"];
    NSString *languaStr = [appLanguages objectAtIndex:0];
    languaStr = [languaStr substringToIndex:2];
    if ([languaStr isEqualToString:@"zh"])
    {
        national = @"zh";
    }
    else
